...
`barricade_rl.m2_acceptance` is the evidence-producing M2 gate. It pins the config, corpus, checkpoint, and 800-simulation budget by hash in a manifest; validates exact binary labels; reconstructs complete solver-optimal action sets for the supported no-wall tablebase; and reports EMA value sign accuracy, phase splits, prediction bias, calibration, raw-policy optimal-set accuracy, and deterministic MCTS optimal-set accuracy. Optimal sets and per-position searches are appended and flushed as JSONL, allowing an interrupted spawned-worker run to resume without changing results. A limited run can diagnose the path but cannot pass the 5,000-position criterion. The summary also refuses to infer the supervisor's full-wall initial second-player-win condition or monotone ladder history from ordinary checkpoint gating: both remain explicitly blocked until their respective proof and fixed-ladder records exist.
