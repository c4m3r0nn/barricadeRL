...
M0 passed its correctness, independent differential, perft, and throughput gates on 2026-07-05, and M1 passed on 2026-07-07. M2 is active: the exact validation corpus remains the formal blocker before sustained training, with multi-process inference scaling still to follow.
...
The default spec is 5x5 with 3 walls per player and an 80-ply cap. The action encoding preserves the production semantics: 12 canonical pawn actions followed by horizontal and vertical wall anchors. On 5x5 that gives 44 actions: `12 + 2 * 4 * 4`.
...
`barricade_rl.az_gating` wraps EMA networks in deterministic evaluation MCTS and compares each candidate with the incumbent over exactly 200 colour-balanced games at 800 simulations with no root noise or move sampling. A candidate is promoted at a score rate of at least 0.55. Promoted checkpoints are copied into an append-only gated archive with a JSONL manifest; rejected candidates remain auditable but cannot replace the self-play network.

`barricade_rl.az_pipeline` is the local correctness-first coordinator. Each cycle generates replay using only the gated incumbent, trains a bounded candidate phase, persists the candidate, runs the gate, archives promotions, and restores the complete incumbent learner checkpoint after rejection. The CLI requires a passing oracle audit before starting. This synchronous implementation pins lifecycle semantics before the same contracts are moved behind multi-process workers and a batched inference server.

`barricade_rl.training_readiness` is the proper-training preflight. It runs the oracle corpus audit and imports and validates replay, MCTS, network, self-play, and learner contracts. Its remaining formal blocker is the 5,000-position exact corpus; production orchestration and gating remain engineering work before an unattended run.
