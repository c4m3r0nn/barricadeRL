...
`barricade_rl.az_pipeline` is the local correctness-first coordinator. It persists monotonically increasing cycle indices; derives separate deterministic self-play, gate-start, and gate-match seeds per cycle; gives every self-play game and rejected candidate a collision-free identity; and records self-play cap fraction and scoring scheme in `cycles.jsonl`. If the cap fraction exceeds 5% for three consecutive cycles, the following cycle automatically switches to recorded shortest-path adjudication. Each cycle otherwise generates replay using only the gated incumbent, trains a bounded candidate phase, persists the candidate, runs the gate, archives promotions, and restores the complete incumbent learner checkpoint after rejection. The CLI requires a passing oracle audit before starting.
