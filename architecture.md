...
M0 passed its correctness, independent differential, perft, and throughput gates on 2026-07-05, and M1 passed on 2026-07-07. M2 is active: the 5,000-position exact validation corpus and proper-training preflight pass, proper training is running, and a formal resumable acceptance evaluator is implemented. The measured value head, full-wall initial proof, full searched-policy corpus, and fixed-ladder convergence evidence do not yet pass the milestone.
...
`barricade_rl.az_parallel` implements the production `spawned-ordered-games-v1` worker protocol. Self-play workers load the gated checkpoint once and gating workers load the candidate/incumbent pair once. The parent derives a stable seed for every self-play game or reproduces the serial gate's complete game-seed sequence before dispatch. Workers are constrained to one Accelerate/OpenMP thread, never share mutable replay or search trees, and return complete records. Ordered `executor.map` collection makes replay order and paired-gate aggregation independent of completion order and worker count. Tests compare one-worker, two-worker, and original serial gate results exactly.

`barricade_rl.az_pipeline` is the local correctness-first coordinator. It persists monotonically increasing cycle indices; derives separate deterministic self-play, gate-start, and gate-match seeds per cycle; gives every self-play game and rejected candidate a collision-free identity; and records self-play cap fraction, scoring scheme, learner device, worker counts/protocol, per-phase timing and games/hour, requested/completed learner steps, final-step loss components, root-policy entropy, learning rate, replay-consumption ratio, and raw/EMA development-validation deltas in `cycles.jsonl`. If the cap fraction exceeds 5% for three consecutive cycles, the following cycle switches to recorded shortest-path adjudication for the remainder of that run, preventing scoring-scheme oscillation after cap rates recover. Each cycle generates replay using only the gated incumbent, computes the legal learner-step headroom from the resulting replay size, clamps oversized requests to the four-samples-per-position limit, advances the continuous learner from its preceding candidate checkpoint, persists the next candidate, evaluates both raw and EMA weights without mutating either, runs the gate using EMA as required, and archives promotions. A rejection leaves the self-play incumbent unchanged but does not discard learner weights, momentum, EMA, step, or RNG state; cycle provenance records the learner input and incumbent separately. The CLI requires a passing oracle audit before starting.

`barricade_rl.apple_benchmark` is a read-only performance probe for choosing the Apple Silicon scaling path without changing training semantics. It compares EMA inference across the production NumPy implementation and equivalent PyTorch CPU/MPS graphs, reports output parity and both compute-only and device-round-trip throughput, separates learner cold-step latency from steady-state throughput, and measures deterministic independent-MCTS scaling with spawned worker processes. It SHA-256 hashes the source checkpoint and replay before and after every command. Batched Metal inference is not yet integrated into MCTS: parallel game workers still make single-position NumPy evaluations, while `--device mps` affects the learner.

`barricade_rl.m2_acceptance` is the evidence-producing M2 gate and development checkpoint validator. Formal acceptance pins the config, corpus, checkpoint, and 800-simulation budget by hash in a manifest; validates exact binary labels; reconstructs complete solver-optimal action sets for the supported no-wall tablebase; and reports EMA value sign accuracy, phase splits, prediction bias, calibration, raw-policy optimal-set accuracy, and deterministic MCTS optimal-set accuracy. The lighter checkpoint validator runs network inference with both raw and EMA parameters, records their metric deltas and the remaining `decay ** step` initialization contribution, and shares a corpus-bound optimal-action cache across cycles. It is observational only: the supervisor-mandated EMA remains the sole self-play, gating, and formal-evaluation network. Optimal sets and per-position searches are appended and flushed as JSONL, allowing interrupted spawned-worker work to resume without changing results. A limited acceptance run can diagnose the path but cannot pass the 5,000-position criterion. The summary also refuses to infer the supervisor's full-wall initial second-player-win condition or monotone ladder history from ordinary checkpoint gating: both remain explicitly blocked until their respective proof and fixed-ladder records exist.
